...
- ein klar dokumentierter Unterschied zwischen `action`, freiem `parameter`
  und Widget-`value`
- eine sichtbare Trennung zwischen generiertem UI-Code und handgeschriebener
  Runtime-Logik
- eine reproduzierbare Projektkonfiguration fuer `lv_conf` und Theme

## Bestaetigter Stand dieses Beispiels

Fuer dieses Referenzprojekt wurde der Integrationspfad bereits praktisch
verifiziert:

- der RTOS-Generator trennt den freien Event-`parameter` vom Laufzeitwert eines
  Widgets
- der `portal`-Slider kann dadurch gleichzeitig den Geschwindigkeitswert und
  einen Zusatzparameter wie `WARNING` transportieren
- `lv_conf_project.h` wird im ESP32-Build als Overlay nach `sdkconfig`
  eingebunden
- `theme_project.c` wird im Display-Pfad aktiv verwendet
- Dark Mode funktioniert grundsaetzlich, wenn Screens keine fest verdrahteten
  hellen Standardfarben erzwingen

Fuer die Beispielhardware gilt aktuell:

- LVGL-Farbtiefe: `16 Bit`
- Panel-Farbformat: `RGB565`
- SDL-Vorschau im Projekt: ebenfalls `16 Bit`
